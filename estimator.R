...
#'
#' Adds residual and urn fixed-effect summaries to the ordinary coefficient
#' display. Detailed stage criteria remain available in the fitted object.
#'
#' @param object a `peer_cra` object.
#' @param ... ignored.
#' @export
summary.peer_cra <- function(object, ...) {
  ...
  r <- object$residuals
  cat(sprintf(
    "\nResiduals:  min = %.3f,  Q1 = %.3f,  median = %.3f,  Q3 = %.3f,  max = %.3f\n",
    min(r), quantile(r, 0.25), median(r), quantile(r, 0.75), max(r)))
  if (length(object$alpha_hat) <= 6) {
    cat("Urn fixed effects (alpha_hat):\n")
    print(signif(object$alpha_hat, 4))
  } else {
    cat(sprintf(
      "Urn fixed effects (alpha_hat): %d urns, mean = %.3f, sd = %.3f\n",
      length(object$alpha_hat), mean(object$alpha_hat),
      sd(object$alpha_hat)))
  }
  invisible(object)
}
